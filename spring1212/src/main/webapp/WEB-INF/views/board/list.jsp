	...
	<div class="box-body">
		<table id="p" class="table table-bordered table-hover">
			...
			<c:forEach var="vo" items="${list }">
				<tr>
					<td align="right">${vo.boardnum}&nbsp;</td>
					<td>&nbsp;${vo.boardtitle}</td>
					<td>&nbsp;${vo.nickname}</td>
					<td>&nbsp;${vo.dispdate}</td>
					<td align="right"><span class="badge bg-blue">
							${vo.boardreadcnt}</span>&nbsp;</td>
				</tr>
			</c:forEach>
			<c:if test = "${d != null}">
				<tr id="c">
...
<script>
document.getElementById('the').addEventListener(
		'click', function(e){
			$.ajax({
				url:'the',
				dataType:'json',
				...
				success:function(map){
					...
					//jquery에서 배열을 순회하기
					//첫번째 매개변수는 배열이고 뒤의 매개변수는
					//배열의 데이터를 순회하면서 호출될 함수
					...
					$.each(data, function(){
						output += "<tr>";
						output += "<td align='right'>" + this.boardnum
						+ "&nbsp;</td>";
						output += "<td>&nbsp;" + this.boardtitle + "</td>";
						output += "<td>&nbsp;" + this.nickname + "</td>";
						output += "<td>&nbsp;" + this.dispdate + "</td>";
						output += "<td align='right'><span class='badge bg-blue'>"+
