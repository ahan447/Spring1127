...
include 태그는 URL 과 상관없이 위치만을 가지고 설정 -->
...
파일 업로드를 할 때는 enctype 을 설정 -->
	<form method="post" enctype="multipart/form-data" id="joinform">
		<p align="center">
		<table align="center" width="60%" height="400">
			<tr>
			...
			<tr>
				<td rowspan="8" align="center">
					<p>사진 파일을 업로드 하세요</p> <img id="img" width="350" height="350"
...
<script>
	...
	$.ajax({
		...
		success:function(data){
			if(data.result === 'true'){			
				nicknamecheck = true;
				nicknamedisp.innerHTML = '&nbsp&nbsp사용 가능한 별명';
				nicknamedisp.style.color = 'green';
				
			}else{
				nicknamecheck = false;
				nicknamedisp.innerHTML = '&nbsp&nbsp사용 중인 별명';
	...
	document.getElementById('joinform').addEventListener('submit',function(e){
		if(emailcheck == false){
			emaildisp.innerHTML = "이메일 중복 검사를 통과해야 함";
			emaildisp.style.color = "red";
			email.focus();
		...
		if(nicknamecheck == false){
			nicknamedisp.innerHTML = "별명 중복 검사를 통과해야 함";
			nicknamedisp.style.color = "red";
			nickname.focus();
			//기본 정의된 이벤트 코드를 수행하지 않음
			e.preventDefault();
		...
		if(pw.value.trim().length < 8){
			pwdisp.innerHTML = "비밀번호는 8자 이상이어야 함";
			pwdisp.style.color = 'red';
			pw.focus();
			e.preventDefault();
			return;
		...
		var p4 = /[!@#$%^&*]/;
		if(!p1.test(pwvalue) || !p2.test(pwvalue) || !p3.test(pwvalue) || !p4.test(pwvalue)){
			pwdisp.innerHTML = "비밀번호는 숫자,영어 대소문자, 특수문자 조합으로 만들어져야 함"
			pwdisp.style.color = "red";
			pw.focus();
			e.preventDefault();
			return;
